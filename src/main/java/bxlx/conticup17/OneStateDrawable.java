...
public class OneStateDrawable extends Container<IDrawable> {
    ...
    public OneStateDrawable(RobotStates states, RobotStateTimer timer, int playerNum) {
        ...
        PlayerPointDrawable playerPointDrawable = new PlayerPointDrawable(states, playerNum);
        ...
        Splitter otherStationHolder = Splitter.threeWaySplit(false, centerSize,
                new Splitter(true, separate1, null,
                        new Splitter(true, separate2, new SplitContainer<>(true)
                                .add(playerNum == 1 ? () -> playerPointDrawable : stations.get(2).getAsSupplier())
                                .add(playerNum == 0 ? () -> playerPointDrawable : stations.get(1).getAsSupplier()),
                                null)),
        ...
        for(int i = 0; i < maxStations; ++i) {
            ...
            int iTh = i;
            stations.get(i).setElem(new StationDrawable(effFinal, states, timer, playerNum, iTh));
        }
